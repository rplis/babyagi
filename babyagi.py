#!/usr/bin/env python3
"""
This script sets up the configuration for BabyAGI, an artificial general intelligence system.
It imports necessary libraries and modules, sets up environment variables, and loads extensions.
It also checks if the required environment variables are present and initializes the Llama model if required.
...
#    OBJECTIVE, INITIAL_TASK, LLM_MODEL, INSTANCE_NAME,
# COOPERATIVE_MODE, JOIN_EXISTING_OBJECTIVE
...
if LLM_MODEL.startswith("llama"):
    if can_import("llama_cpp"):
        ...
        assert os.path.exists(LLAMA_MODEL_PATH), (
            "\033[91m\033[1m" + f"Model can't be found." + "\033[0m\033[0m"
        )
...
if LLM_MODEL.startswith("human"):
    print(
        "\033[91m\033[1m" + "\n*****USING HUMAN INPUT*****" + "\033[0m\033[0m"
    )
...
class DefaultResultsStorage:
    ...
    def add(self, task: Dict, result: str, result_id: str):
        ...
        embeddings = (
            llm_embed.embed(result) if LLM_MODEL.startswith("llama") else None
        )
        if (
...
def try_weaviate():
    ...
    WEAVIATE_USE_EMBEDDED = (
        os.getenv("WEAVIATE_USE_EMBEDDED", "False").lower() == "true"
    )
    if (WEAVIATE_URL or WEAVIATE_USE_EMBEDDED) and can_import(
        "extensions.weaviate_storage"
    ):
